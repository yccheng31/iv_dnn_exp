...
#
# nnet config
nn_depth=6     #number of hidden layers (6)
hid_dim=16   #number of units per layer (2048)
...
splice=0           # Temporal splicing (5)
splice_step=1      # Stepsize of the splicing (1 is consecutive splice, 
                   # value 2 would do [ -10 -8 -6 -4 -2 0 2 4 6 8 10 ] splicing when splice==5) (1)
# misc.
verbose=1 # enable per-cache reports
# gpu config
#use_gpu_id=0 # manually select GPU id to run on, (-1 disables GPU) 
use_gpu=yes
# End configuration.

# apply if no feature transform is going to be there
no_feat_transform=false
...
if [ $# != 2 ]; then
   ...
   echo "  --no_feat_transform <bool>       # if set to true, the feature_transform will be set to nothing"
   exit 1;
fi
...
if [ $apply_cmvn == "true" ]; then
  ...
  echo "$nfeat_dimorm_vars" >$dir/norm_vars 
else
...
if [ $no_feat_transform == true ]; then
  feature_transform=""
else
  if [ ! -z "$feature_transform" ]; then
    ...
    nnet-forward ${use_gpu:+ --use-gpu=$use_gpu} \
    ...
    [ -f $dir/final.feature_transform ] && rm $dir/final.feature_transform
...
for depth in $(seq 1 $nn_depth); do
  ...
  if [ "$depth" == "1" ]; then
    ...
    echo utils/nnet/gen_rbm_init.py --dim=${num_fea}:${num_hid} --gauss --vistype=gauss --hidtype=bern
    ...
    rbm-train-cd1-frmshuff --learn-rate=$rbm_lrate_low --l2-penalty=$rbm_l2penalty \
      ...
      --feature-transform=$feature_transform \
      ${use_gpu:+ --use-gpu=$use_gpu} $rbm_extra_opts \
    ...
    if [ ! -f $dir/$depth.cmvn ]; then 
      nnet-forward ${use_gpu:+ --use-gpu=$use_gpu} \
       "nnet-concat --binary=false $feature_transform $dir/$((depth-1)).dbn - |" \
    ...
    #initialize
    echo "Initializing '$RBM.init'"
    echo "utils/nnet/gen_rbm_init.py --dim=${num_hid}:${num_hid} --gauss --vistype=bern --hidtype=bern --cmvn-nnet=$dir/$depth.cmvn"
    ...
    rbm-train-cd1-frmshuff --learn-rate=$rbm_lrate --l2-penalty=$rbm_l2penalty \
      ...
      --feature-transform="nnet-concat --binary=false $feature_transform $dir/$((depth-1)).dbn - |" \
      ${use_gpu:+ --use-gpu=$use_gpu} $rbm_extra_opts \
      $RBM.init "$feats" $RBM 2>$dir/log/rbm.$depth.log || exit 1
    #rbm-train-cd1-frmshuff --learn-rate=$rbm_lrate --l2-penalty=$rbm_l2penalty \
    #  --num-iters=$rbm_iter --drop-data=$rbm_drop_data --verbose=$verbose \
