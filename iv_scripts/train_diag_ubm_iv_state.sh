...
num_iters_init=20
initial_gauss_proportion=0.125 # Start with 1/8 the target number of Gaussians
...
if [ $# != 4 ]; then
  echo "Usage: $0  <data> <state_suffix> <num-gauss> <output-dir>"
  echo " e.g.: $0 data/train 0 1024 exp/diag_ubm"
  echo "Options: "
...
all_feats="scp:$data/trainJPGs_UE_state_${suffix}.scp"
#feats="scp:$data/JOB/trainJPGs_UE_state_${suffix}.scp"
#cp $all_feats $feats
...
if [ $stage -le -2 ]; then
  ...
  echo $cmd $parallel_opts $dir/log/gmm_init.log \
    gmm-global-init-from-feats --binary=false --num-threads=$num_threads --num-frames=$num_frames \
     --min-gaussian-weight=$min_gaussian_weight \
     --num-gauss=$num_gauss --num-gauss-init=$num_gauss_init --num-iters=$num_iters_init --verbose=9 \
...
if [ $stage -le -1 ]; then
  ...
  $cmd $dir/log/gselect.JOB.log \
    gmm-gselect --verbose=9 --n=$num_gselect $dir/0.dubm "$all_feats" \
	ark:$dir/state_${suffix}_gselect.ark || exit 1;
...
for x in `seq 0 $[$num_iters-1]`; do
  ...
  # Accumulate stats.
    ...
    $cmd $dir/log/state_${suffix}_acc.$x.log \
    #  gmm-global-acc-stats --binary=false "--gselect=ark,s,cs:gunzip -c $dir/gselect.JOB.gz|" \
      gmm-global-acc-stats --verbose=9 --binary=false "--gselect=ark:$dir/state_${suffix}_gselect.ark" $dir/$x.dubm "$all_feats" $dir/$x.acc || exit 1;
    ...
    $cmd $dir/log/update.$x.log \
      #gmm-global-est --binary=false $opt --min-gaussian-weight=$min_gaussian_weight $dir/$x.dubm "gmm-global-sum-accs - $dir/$x.*.acc|" \
      gmm-global-est --verbose=9 --binary=false $opt --min-gaussian-weight=$min_gaussian_weight $dir/$x.dubm $dir/$x.acc \
      $dir/$[$x+1].dubm || exit 1;
...
#rm $dir/gselect.*.gz
#rm $dir/state_${suffix}_gselect.ark
