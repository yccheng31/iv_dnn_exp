...
dims=($num_fea `echo $num_hid | awk -F ':' '{for (i=1;i<=NF;i++) printf("%s ", $i)}'`)
...
for depth in $(seq 1 $nn_depth); do
  ...
  if [ "$depth" == "1" ]; then
    ...
    utils/nnet/gen_rbm_init.py --dim=${num_fea}:${dims[$depth]} --gauss --vistype=gauss --hidtype=bern > $RBM.init || exit 1
    #pre-train
    echo "Pretraining '$RBM' (reduced lrate and 2x more iters)"
    rbm-train-cd1-frmshuff-largeutt --learn-rate=$rbm_lrate_low --l2-penalty=$rbm_l2penalty \
    ...
    #initialize
    echo "Initializing '$RBM.init'"
    utils/nnet/gen_rbm_init.py --dim=${dims[$depth-1]}:${dims[$depth]} --gauss --vistype=bern --hidtype=bern --cmvn-nnet=$dir/$depth.cmvn > $RBM.init || exit 1
    #pre-train
    echo "Pretraining '$RBM'"
    rbm-train-cd1-frmshuff-largeutt --learn-rate=$rbm_lrate --l2-penalty=$rbm_l2penalty \
